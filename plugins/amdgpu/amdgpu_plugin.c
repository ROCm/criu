...
int amdgpu_plugin_handle_device_vma(int fd, struct stat *st_buf)
{
...
int amdgpu_plugin_dump_file(int fd, int id)
{
	...
	for (int i = 0; i < helper_args.num_of_bos; i++)
	{
		...
		if ((bo_bucket_ptr)[i].bo_alloc_flags &
			...
			if (mem_fd < 0) {
				pr_perror("Can't open %s for pid %d", fname, helper_args.task_pid);
				free (fname);
				ret = -1;
				goto failed;
			}
			...
			if (lseek (mem_fd, (off_t) (bo_bucket_ptr)[i].bo_addr, SEEK_SET) == -1) {
				pr_perror("Can't lseek for bo_offset for pid = %d", helper_args.task_pid);
				ret = -1;
				goto failed;
			}
