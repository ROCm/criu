...
int sdma_copy_bo(struct kfd_criu_bo_buckets *bo_buckets,
...
void *dump_bo_contents(void *_thread_data)
{
	...
	for (i = 0; i < thread_data->num_of_bos; i++) {
		...
		if (bo_buckets[i].bo_alloc_flags & KFD_IOC_ALLOC_MEM_FLAGS_VRAM) {
			...
			if (!sdma_copy_bo(bo_buckets, bo_info_test,
					i, h_dev, SDMA_OP_VRAM_READ)) {
		...
		if (bo_info_test[i]->bo_alloc_flags & KFD_IOC_ALLOC_MEM_FLAGS_PUBLIC) {
			...
			if (lseek (mem_fd, (off_t) bo_buckets[i].bo_addr, SEEK_SET) == -1) {
				pr_perror("Can't lseek for bo_offset for pid = %d", thread_data->pid);
				ret = -errno;
				goto exit;
			}
...
int amdgpu_plugin_dump_file(int fd, int id)
{
	...
	/* call dumper ioctl, pass num of BOs to dump */
		...
		pr_perror("amdgpu_plugin: failed to call kfd ioctl from plugin dumper for fd = %d", major(st.st_rdev));
		xfree(bo_bucket_ptr);
		return -1;
	}
...
int amdgpu_plugin_restore_file(int id)
{
	...
	struct kfd_criu_bo_buckets *bo_bucket_ptr = NULL;;
	__u64 *restored_bo_offsets_array = NULL;
	struct kfd_criu_q_bucket *q_bucket_ptr = NULL;
...
fail:
	...
	if (kmtIoctl(fd, AMDKFD_IOC_CRIU_RESTORER, &args) == -1) {
		pr_perror("amdgpu_plugin: failed to call kfd ioctl from plugin restorer for id = %d", id);
		fd = -EBADFD;
		goto clean;
	}
