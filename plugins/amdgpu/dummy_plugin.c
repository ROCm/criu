 ...
 */
int dummy_plugin_update_vmamap(const char *old_path, char *new_path, const uint64_t addr,
				const uint64_t old_offset, uint64_t *new_offset)
{
	uint64_t temp = 100;

	*new_offset = temp;
	pr_info("dummy_plugin: old_pgoff= 0x%lx new_pgoff = 0x%lx old_path = %s new_path = %s addr = 0x%lx\n",
