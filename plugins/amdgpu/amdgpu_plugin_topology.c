...
static struct id_map *maps_add_cpu_entry(struct device_maps *maps, const uint32_t src_id,
	...
	list_for_each_entry(node, &sys->nodes, listm_system) {
		...
		if (!NODE_IS_GPU(node)) {
			...
			pr_info("     mem_banks_count:%u caches_count:%d lds_size_in_kb:%u\n",
					node->mem_banks_count, node->caches_count,
					node->lds_size_in_kb);
			...
			pr_info("     num_sdma_xgmi_engines:%u num_sdma_queues_per_engine:%u\n",
					node->num_sdma_xgmi_engines,
					node->num_sdma_queues_per_engine);
			pr_info("     num_cp_queues:%u fw_version:%u sdma_fw_version:%u\n",
					node->num_cp_queues, node->fw_version,
					node->sdma_fw_version);
		}
		list_for_each_entry(iolink, &node->iolinks, listm) {
			if (!iolink->valid)
				continue;
			...
			pr_info("     iolink type:%s node-to:%d (0x%04X) node-from:%d bi-dir:%s\n",
				link_type(iolink->type), iolink->node_to_id,
				iolink->node_to->gpu_id,
...
void topology_free(struct tp_system *sys)
{
	while (!list_empty(&sys->nodes)) {
		...
		while (!list_empty(&node->iolinks)) {
			struct tp_iolink *iolink = list_first_entry(&node->iolinks,
								    struct tp_iolink, listm);
	...
	while (!list_empty(&sys->xgmi_groups)) {
		struct tp_p2pgroup *p2pgroup = list_first_entry(&sys->xgmi_groups,
								struct tp_p2pgroup, listm_system);
	...
	list_for_each_entry(node, &sys->nodes, listm_system) {
		...
		list_for_each_entry(iolink, &node->iolinks, listm) {
			...
			if (iolink->type == TOPO_IOLINK_TYPE_XGMI) {
				group = sys_get_p2pgroup_with_gpu_id(sys, iolink->type,
								     node->gpu_id);
				if (!group) {
 ...
 */
static bool map_device(struct tp_system *src_sys, struct tp_system *dest_sys,
		       struct tp_node *src_node, struct tp_node *dest_node,
		       struct device_maps *maps, struct device_maps *new_maps)
{
		...
		if (!NODE_IS_GPU(src_iolink->node_to)) {
			...
			if (!kfd_numa_check) {
				...
				if (dest_cpu_node_id == INVALID_CPU_ID)
					dest_cpu_node_id = maps_get_dest_cpu(new_maps,
									src_iolink->node_to->id);

				if (dest_cpu_node_id == INVALID_CPU_ID) {
					struct tp_iolink *dest_iolink;
					list_for_each_entry(dest_iolink,
							    &dest_node->iolinks,
							    listm) {

						if (iolink_match(src_iolink, dest_iolink) &&
						    !maps_dest_cpu_mapped(maps,
								dest_iolink->node_to->id) &&
						    !maps_dest_cpu_mapped(new_maps,
								dest_iolink->node_to->id)) {

							if (!maps_add_cpu_entry(new_maps,
									src_iolink->node_to->id,
									dest_iolink->node_to->id))
					...
					struct tp_iolink *dest_iolink =
						node_get_iolink_to_node_id(dest_node,
									   src_iolink->type,
									   dest_cpu_node_id);
 ...
 */
static bool map_devices(struct tp_system *src_sys, struct tp_system *dest_sys,
			struct list_head *src_nodes, struct list_head *dest_nodes,
			...
			struct device_maps *maps)
{
	...
	list_for_each_entry_safe(dest_group, dest_group_tmp, dest_xgmi_groups, listm_system) {
		...
		if (map_devices(src_sys, dest_sys, &src_nodes, &dest_nodes, maps)) {
			...
			if (match_xgmi_groups(src_sys, dest_sys, src_xgmi_groups,
					      dest_xgmi_groups, maps)) {
			 ...
			 struct device_maps *maps)
{
	...
	if (src_sys->num_xgmi_groups) {
		if (!match_xgmi_groups(src_sys, dest_sys, &src_sys->xgmi_groups,
				       &dest_sys->xgmi_groups, maps)) {
