...
DECLARE_PLUGIN_HOOK_ARGS(CR_PLUGIN_HOOK__HANDLE_DEVICE_VMA, int fd, struct stat *stat);
...
typedef int (cr_plugin_handle_device_vma_t)(int fd, struct stat *stat);
typedef int (cr_plugin_dump_ext_link_t)(int index, int type, char *kind);
typedef int (cr_plugin_update_vma_map_t)(const char* old_path, char *new_path,
